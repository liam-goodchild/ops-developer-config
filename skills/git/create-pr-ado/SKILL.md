...
Use the bundled Python helper for deterministic branch, default branch, existing PR, work item, and diff-stat checks. Use the LLM only for judgement: deriving the concise PR title/description from the diff and asking when the branch prefix is unclear.
...
3. Use `branch_mapping` and `templates` to draft a PR title and description. If the branch contains a work item number, include it in `work_items`.
