...
def inspect(t: str) -> dict[str,Any]:
    p=target_dir(t); az=exe('az'); remote=remote_origin(p); ado=parse_ado(remote); branch=(git(p,'branch','--show-current').stdout.strip() if git_root(p) else None); default=None; existing=[]
    if az and ado:
        org=f"https://dev.azure.com/{ado['org']}"; cp=run([az,'repos','show','--org',org,'--project',ado['project'],'--repository',ado['repo'],'--query','defaultBranch','-o','tsv']); default=cp.stdout.strip().replace('refs/heads/','') if cp.returncode==0 else None
        if branch:
            pr=run([az,'repos','pr','list','--org',org,'--project',ado['project'],'--repository',ado['repo'],'--source-branch',branch,'--status','active','-o','json'])
...
def apply(t, plan_path, dry):
    ...
    org=f"https://dev.azure.com/{ado['org']}"; cmd=[az,'repos','pr','create','--org',org,'--project',ado['project'],'--repository',ado['repo'],'--target-branch',plan.get('target_branch','main'),'--title',title,'--description',desc,'-o','json']
    for wi in plan.get('work_items',[]) or []: cmd.extend(['--work-items',str(wi)])
    if dry: return {'dry_run':True,'would_run':cmd[1:]}
    cp=run(cmd,check=True); data=json.loads(cp.stdout); return {'pullRequestId':data.get('pullRequestId'),'url':f"https://dev.azure.com/{ado['org']}/{ado['project']}/_git/{ado['repo']}/pullrequest/{data.get('pullRequestId')}"}
